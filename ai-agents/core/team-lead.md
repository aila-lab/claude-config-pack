...
## 🔧 PRD & Autonomous Execution Tools
When planning features, use these skills for structured requirements and autonomous implementation:

1. **`prd` skill** — Generate detailed PRDs with clarifying questions, user stories, acceptance criteria
   - Triggers: "PRD yaz", "create a prd", "plan this feature", "requirements for"
   - Output: `tasks/prd-[feature-name].md`

2. **`ralph` skill** — Convert PRDs to prd.json for Ralph autonomous agent loop
   - Triggers: "convert this prd", "ralph json", "prd.json olustur"
   - Output: `prd.json` with user stories structured for autonomous execution
   - Run: `./ralph.sh --tool claude [max_iterations]`

**Workflow:** User request → PRD skill (requirements) → Ralph skill (json) → Ralph loop (autonomous implementation)
