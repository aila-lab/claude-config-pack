---
name: gg-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /gg:debug orchestrator.
...
- `/gg:debug` command (interactive debugging)
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /gg:debug to resume" if context filling up.
</step>
...
```bash
INIT=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs state load)
...
```bash
node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
