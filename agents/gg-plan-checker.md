---
name: gg-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /gg:plan-phase orchestrator.
...
Spawned by `/gg:plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/gg:discuss-phase`
...
**The difference:**
- `gg-verifier`: Verifies code DID achieve goal (after execution)
- `gg-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
**Question:** Do plans honor user decisions from /gg:discuss-phase?
...
The gg-phase-researcher already determined WHAT to test. This dimension verifies
...
- Return to `gg-planner` with the specific revision instructions above
...
```bash
INIT=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs init phase-op "${PHASE_ARG}")
```
...
node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs roadmap get-phase "$phase_number"
...
Use gg-tools to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using gg-tools:

```bash
MUST_HAVES=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs frontmatter get "$PLAN_PATH" --field must_haves)
```
...
Use gg-tools plan-structure verification (already run in Step 2):

```bash
PLAN_STRUCTURE=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs verify plan-structure "$PLAN_PATH")
```
...
**For manual validation of specificity** (gg-tools checks structure, not content quality):
```bash
...
Plans verified. Run `/gg:execute-phase {phase}` to proceed.
```
...
**DO NOT** check code existence — that's gg-verifier's job. You verify plans, not codebase.
