---
name: gg-phase-researcher
description: Researches how to implement a phase before planning. Produces RESEARCH.md consumed by gg-planner. Spawned by /gg:plan-phase orchestrator.
...
Spawned by `/gg:plan-phase` (integrated) or `/gg:research-phase` (standalone).
...
<downstream_consumer>
Your RESEARCH.md is consumed by `gg-planner`:
...
```bash
node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs websearch "your query" --limit 10
```
...
- **Phase-complete gate:** Full suite green before `/gg:verify-work` runs
...
```bash
INIT=$(node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs init phase-op "${PHASE}")
```
...
```bash
node C:/Users/ymeli/.claude/good-guys/bin/gg-tools.cjs commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
